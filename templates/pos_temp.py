...
parser = argparse.ArgumentParser(description='convert lat lon to gazebo XYZ world location')
parser.add_argument('--lat-lon', type=float, nargs='+',
                    help='lat lon location to be converted')
...
ned = navpy.lla2ned(lat_in, lon_in, offset, center_lat_lon[0], center_lat_lon[1], offset)
print(ned)
...
print(height, width)
elevation = elevation_map[int(height/2)-int(x)][int(width/2) + int(y)]*(terrain_delta/255) + offset
print(elevation)